...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public boolean simpan(MhsModel mm){
        ...
        long stts = dbase.insert(TABLE_NAME, null, cv);
    ...
    public boolean hapus(int id){
        SQLiteDatabase dbase = this.getWritableDatabase();
        int rowAffected = dbase.delete(TABLE_NAME, "id = ?", new String[]{String.valueOf(id)});
    ...
    public boolean ubah(MhsModel mm){
        ...
        cv.put(COLUMN_NIM, mm.getNim());
        cv.put(COLUMN_NO_HP, mm.getNoHp());
        int rowAffected = dbase.update(TABLE_NAME, cv, "id = ?", new String[]{String.valueOf(mm.getId())});
